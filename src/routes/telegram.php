...
Route::middleware('auth.telegram')->group(function () {
    ...
    // Сообщения - РАЗНЫЕ endpoint'ы чтобы избежать конфликта
    Route::get('/dialogs/{dialogId}/messages', [MessageController::class, 'index'])->name('messages.index'); // GET - получение
    Route::post('/dialogs/{dialogId}/send', [MessageController::class, 'store'])->name('messages.send'); // POST - отправка
    Route::get('/dialogs/{dialogId}/messages/{messageId}', [MessageController::class, 'show'])->name('messages.show'); // GET - одно сообщение
});
